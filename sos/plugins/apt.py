...
class Apt(Plugin, DebianPlugin, UbuntuPlugin):
    ...
    def setup(self):
        ...
        dpkg_result = self.call_ext_prog("dpkg-query -W -f='${binary:Package}\t${status}\n'")
        pdb.set_trace()
        dpkg_output=dpkg_result['output'].splitlines()
        pkg_list=[v.split('\t')[0] for v in dpkg_output if 'ok installed' in v]
        self.add_cmd_outputs([
            "apt-get check",
            "apt-config dump",
            "apt-cache stats",
            "apt-cache policy",
            "apt-cache policy %s" % pkg_list
        ])
